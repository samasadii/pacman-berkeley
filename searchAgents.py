...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        for corner in list(self.corners):
            if (self.startingPosition != corner):
                self.notVisitedCorners.append(corner)
...
def cornersHeuristic(state, problem):
    ...
    while notVisitedCorners != []:
        ...
        for corner in notVisitedCorners:
            distance = abs(location[0] - corner[0]) + abs(location[1] - corner[1])
            if (distance < minimum):
                minimum = distance
                j = i
            i += 1
        ...
        h += minimum
        location = notVisitedCorners[j]
        notVisitedCorners.remove(location)
